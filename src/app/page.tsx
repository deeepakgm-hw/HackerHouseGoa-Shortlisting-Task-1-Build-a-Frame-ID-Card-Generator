...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#0b4f30] text-[#faf8f0] flex flex-col items-center relative overflow-hidden font-serif select-none">
      ...
      {/* Header */}
      <header className="w-full max-w-6xl px-6 py-6 flex items-center justify-between border-b-3 border-[#0a2e1d] z-20 bg-[#0b4f30]">
        ...
        <div className="flex items-center gap-6">
          <a
            ...
            className="text-xs px-3 md:px-4 py-2 border-2 border-[#0a2e1d] bg-[#faf8f0] hover:bg-[#ff007f] hover:text-[#faf8f0] text-[#0b4f30] font-black font-mono tracking-wider transition-all duration-200 flex items-center gap-1.5 shadow-[2px_2px_0px_0px_#0a2e1d]"
          >
            <Compass className="w-3.5 h-3.5 text-cyan-700" />
            <span>Join Builder Wall</span>
          </a>
        </div>
      </header>

      {/* Main Section */}
      <main className="w-full max-w-6xl px-6 py-6 md:py-10 flex-1 flex flex-col items-center justify-center z-20">
        <Generator />
      </main>
      ...
      {/* Footer */}
      <footer className="w-full text-center text-[#faf8f0]/40 text-xs py-8 border-t border-[#faf8f0]/10 z-20 font-mono">
        <p>© 2026 Hacker House Goa. Built with ❤️ for the developer community.</p>
      </footer>
    </div>
  );
}
